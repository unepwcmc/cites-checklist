...
#loxodonta
order = Taxon.create(:rank_id => Rank.find_by_name('Order'),
  :scientific_name => 'Proboscidea', :parent_id => klass.id)
order.institutions = Institution.all
family = Taxon.create(:rank_id => Rank.find_by_name('Family'),
  :scientific_name => 'Elephantidae', :parent_id => order.id)
family.institutions = Institution.all
genus = Taxon.create(:rank_id => Rank.find_by_name('Genus'),
...
['has_part', 'is_part_of'].each do |relationship|
